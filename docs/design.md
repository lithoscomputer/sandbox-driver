...
| `Git` | clone, status, add, commit, push, pull, branches, checkout, set_ambient_credentials — low-level plumbing only; providers hide native, derived, or hybrid selection; per-call credentials for the facet's own network operations, a credential store for the workload's git commands; over the wire, clone is the plugin's own (`git/clone`) and the rest derive on the host | derived | derived | hybrid: native clone, derived remainder | derived |
