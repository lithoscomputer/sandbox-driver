...
A second process reaches that registry's sandboxes through `HostProvider::observe_registry(directory)`: a temporary provider that also reads the owner's records and never writes them. `attach` by id and `attach_directory` by workspace path return read-only handles that describe the sandbox from the owner's current record (id, labels, ownership, state), run commands in the workspace, and read and write its files; `start`, `stop`, and `delete` fail with `Error::ReadOnly`, so the owner stays the only writer. The handle's process groups are the observer's own and admit work whatever the owner's state, so a stopped sandbox's retained workspace stays usable; they end when the observer drops. A directory no record names is `NotFound` by path and is still designated by its path-derived id.

On Linux and macOS, each exec runs under a sentinel in its own process group. The provider holds the unreaped sentinel until stop, so live signals cannot reach a recycled id. The sentinel publishes its group record before checking its generation's fence marker and starting the workload. An in-group watcher observes that marker and kills its own group even after the plugin dies. Once the workload has exited and its status is written, the idle sentinel also kills its own group when its owning provider process is gone, so a provider that is killed rather than stopped leaves no process behind; a running workload still survives its owner so recovery can fence it. Recovery writes markers first and then observes process death through procfs or libproc. It never signals a saved id. A group that does not drain within five seconds reports provider code `fence_leaked`; stop and delete remain retryable after the operator ends it. Stream output and stdin use the same provider exec implementation.
