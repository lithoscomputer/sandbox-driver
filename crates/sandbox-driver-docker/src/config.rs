...
//!
//! [`SandboxSpec::provider_config`](sandbox_driver::SandboxSpec) stays an
//! opaque JSON value on the portable spec — it crosses the plugin wire that
//! way, and every provider defines its own shape. This module is the
//! Docker shape, so an in-process caller constructs it type-checked and
//! converts with [`DockerProviderConfig::into_value`] instead of spelling
//! string keys; a typo is a compile error, not a runtime `InvalidSpec`. The
//! provider parses the value back through the same types, so the two can
//! never drift.
//!
...
/// reachable from the sandbox by `name`. See the sidecar module docs for
/// the readiness rule.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Sidecar {
...
impl Health {
    ...
    pub(crate) fn to_config(&self) -> HealthConfig {
        HealthConfig {
